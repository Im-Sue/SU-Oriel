...
export interface ValidationIssue { path: string; actual: unknown; expected: string; }
export interface ValidationResult { ok: boolean; issues: ValidationIssue[]; }

const SCHEMA_NAME = "dev-task";
const ROOT_TYPE: string = "markdown_with_frontmatter";
...
const FIELD_RULES: Array<Record<string, unknown>> = [
  {
    "name": "doc_type",
    "type": "literal",
    "value": "dev_task"
...
const BODY_RULE: null | { type?: string; required?: boolean; [key: string]: unknown } = {
...
function hasUsefulMarkdown(value: unknown): boolean {
  if (typeof value !== "string") return false;
  const trimmed = value.trim();
  return trimmed.length >= 50 && /(^|\n)\s*(#{1,6}\s+|[-*]\s+)/.test(trimmed);
}

function validateField(rule: Record<string, unknown>, value: unknown, issues: ValidationIssue[]): void {
...
function selectPayload(input: unknown): { frontmatter: Record<string, unknown>; body: unknown } {
...
export function validateDevTask(input: unknown): ValidationResult {
