/**
 * C2 一次性迁移：把现有 docs/.ccb/requirements/active/*.md 迁到 docs/02_需求设计/。
 *
 * 默认 dry-run；不会自动执行。推荐先 dry-run review，再 --apply。
 *
 * 用法：
 *   pnpm tsx scripts/migrate-ccb-requirements-to-docs02.ts --project=<projectId>
 *   pnpm tsx scripts/migrate-ccb-requirements-to-docs02.ts --project-root=/path/to/repo --apply
 *   pnpm tsx scripts/migrate-ccb-requirements-to-docs02.ts --project=<projectId> --apply --move
 */
...
function ensureRequirementDocType(content: string): string {
  const matched = content.match(/^(---\r?\n)([\s\S]*?)(\r?\n---[\s\S]*)$/);
  if (!matched || /^\s*doc_type\s*:/m.test(matched[2])) return content;
  const lines = matched[2].split(/\r?\n/);
  const titleIndex = lines.findIndex((line) => /^\s*title\s*:/.test(line));
  const insertAt = titleIndex === -1 ? lines.length : titleIndex + 1;
  lines.splice(insertAt, 0, "doc_type: requirement");
  return `${matched[1]}${lines.join("\n")}${matched[3]}`;
}

function requirementFileName(title: string, requirementId: string, namingRule: string): string {
  const subject = `${slugify(title) || "requirement"}-${requirementId.slice(-6)}`;
...
async function main(): Promise<void> {
  ...
  const sourceRoot = join(projectRoot, "docs", ".ccb", "requirements", "active");
  const requirement = getDocsStructureResolver().resolveDocType("requirement");
  const targetRoot = join(projectRoot, requirement.directory);
  const sourceFiles = await collectMarkdownFiles(sourceRoot);
  const rows: MigrationRow[] = [];
  const writes: Array<{ sourcePath: string; targetPath: string; content: string; row: MigrationRow }> = [];
  ...
  for (const sourcePath of sourceFiles.sort()) {
    ...
    if (!id) {
      rows.push({
        ...
        hashMatch: false,
        action: "skip_invalid",
        issue: "missing frontmatter.id"
      });
      continue;
    }

    const title = frontmatter.title?.trim() || basename(sourcePath, ".md");
    const targetPath = join(targetRoot, requirementFileName(title, id, requirement.namingRule));
    const content = ensureRequirementDocType(rawContent);
    ...
    const row: MigrationRow = { sourcePath, targetPath, title, exists, hashMatch, action };
    rows.push(row);
    if (action === "write") writes.push({ sourcePath, targetPath, content, row });
  }

  console.log(`=== ${options.apply ? "APPLY" : "dry-run"} · migrate-ccb-requirements-to-docs02 ===`);
